...
\begin{enumerate}
    ...
    \begin{proof}
        ...
        \begin{align}
            ...
            \begin{cases}
                f(\abs{x})
                + \sum_{i=1}^{N}\int_0^t\frac{B_{s\wedge T_\ep}^i}{\abs{B_{s\wedge T_\ep}}^2}dB_s^i
                + \frac{1}{2}\sum_{i=1}^{N}\int_0^t\frac{1}{\abs{B_{s\wedge T_\ep}}^2}\left(1-\frac{2(B_{s\wedge T_\ep}^i)^2}{\abs{B_{s\wedge T_\ep}}^2}\right)ds & N=2, \\
                f(\abs{x})
            ...
            \begin{cases}
                f(\abs{x})
                + \sum_{i=1}^{N}\int_0^t\frac{B_{s\wedge T_\ep}^i}{\abs{B_{s\wedge T_\ep}}^2}dB_s^i & N=2, \\
                f(\abs{x})
                + \sum_{i=1}^{N}\int_0^t\frac{(2-N)B_{s\wedge T_\ep}^i}{\abs{B_{s\wedge T_\ep}}^N}dB_s^i & N\ge3
            \end{cases}
        \end{align}
        が成り立ち，任意の $N\ge2$ と $1\le i\le N$ に対し $\frac{B_{t\wedge T_\ep}^i}{\abs{B_{t\wedge T_\ep}}^N}\in L_{\mathrm{loc}}^2(B_t^i)$ であるので $f(\abs{B_{t\wedge T_\ep}})$ はCLM.
    \end{proof}
